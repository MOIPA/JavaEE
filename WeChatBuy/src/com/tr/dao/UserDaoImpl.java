...
//                    urlLists.add(parentDir + "/" + fileName);
        ...
        logger.info("上传二维码处理完毕");
        return payCodeUrl;
    }

    @Override
    public int saveUserPayCodeUrl(String payCodeUrl, String orderId) {
        String sql = "insert into orderBarcode(orderid,barcodeurl)values(?,?)";
        System.out.println(payCodeUrl + ":" + orderId);
        QueryRunner queryRunner = BaseDataUtil.getQueryRunner();
        try {
            return queryRunner.update(sql, orderId, payCodeUrl);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return 0;
